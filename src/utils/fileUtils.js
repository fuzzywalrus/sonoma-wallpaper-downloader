...
export const readLocalizedStrings = async (localPath, fallbackUrl) => {
  ...
  try {
    ...
    if (window.electron) {
      try {
        // Try to read the file using IPC bridge to main process
        console.log('Attempting to read file via main process:', localPath);
        const data = await window.electron.fs.readFile(localPath);
        console.log('File read successful, parsing data...');
        parsedData = await window.electron.bplistParser.parseBuffer(data);
...
const fetchAndParsePlist = async (url) => {
  try {
    ...
    // In Electron context with IPC bridge
    if (window.electron && window.electron.bplistParser) {
      // Convert ArrayBuffer to Buffer or Uint8Array that bplist parser can handle
      const buffer = new Uint8Array(arrayBuffer);
      console.log('Parsing bplist via IPC...');
      return window.electron.bplistParser.parseBuffer(buffer);
    } else {
...
export const readEntriesJson = async (localPath, fallbackUrl) => {
  ...
  try {
    ...
    if (window.electron && isLocalFileAccess) {
      // For local file access in Electron, use the IPC bridge
      try {
        // Convert file:/// URL to actual file path
        const filePath = decodeURIComponent(localPath.replace('file:///', '/'));
        console.log('Trying to read local file via IPC:', filePath);
        const data = await window.electron.fs.readFile(filePath, { encoding: 'utf8' });
        // Parse the JSON manually
        return JSON.parse(data);
      } catch (localError) {
        console.log('Local file read failed via IPC:', localError.message);
        console.log('Trying fallback URL...');
      }
    }
    
    // Fallback to fetch API (for web or if IPC failed)
    try {
